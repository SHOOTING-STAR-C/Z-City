...
| 触发条件 | 来源 | 说明 |
|---------|------|------|
| 血量低于 **500** | [sv_blood.lua:176](lua/homigrad/organism/tier_1/modules/sv_blood.lua#L176) | 受肾上腺素影响，阈值在 500 附近波动 |
| 意识低于 **0.1** | [sv_pain.lua:92](lua/homigrad/organism/tier_1/modules/sv_pain.lua#L92) | 意识由休克、血量和药物共同控制 |
| 休克超过阈值 | [sv_pain.lua:30](lua/homigrad/organism/tier_1/modules/sv_pain.lua#L30) | `休克 > 2000 × 4 × 止痛药倍率`，即休克 > **8000**（无止痛药时） |
| 血氧低于 **1** | [sv_lungs.lua:349](lua/homigrad/organism/tier_1/modules/sv_lungs.lua#L349) | 窒息/肺部损伤导致 |
| 脑损伤 ≥ **0.3** | [sv_lungs.lua:385](lua/homigrad/organism/tier_1/modules/sv_lungs.lua#L385) | 严重脑损伤直接昏迷 |
...
| 触发条件 | 来源 | 说明 |
|---------|------|------|
| 血量低于 **500** | [sv_organism.lua:469](lua/homigrad/organism/tier_1/sv_organism.lua#L469) | 失血过多无法站立 |
| 意识低于 **0.4** | [sv_pain.lua:96](lua/homigrad/organism/tier_1/modules/sv_pain.lua#L96) | 轻度意识模糊 |
| 脊椎损伤 | [sv_organism.lua:465](lua/homigrad/organism/tier_1/sv_organism.lua#L465) | `canmove = false` |
| 血氧低于 **5** | [sv_lungs.lua:341](lua/homigrad/organism/tier_1/modules/sv_lungs.lua#L341) | 轻度缺氧 |
...
- 昏倒后计时器开始计时
- 计时超过 **2 秒** 且昏倒条件不再满足时自动苏醒

---

## 死亡条件

> 以下全部为玩家数值；NPC 保持原始高难度数值，不受影响。

### 致死路径一览

| 方式 | 条件 | 说明 |
|------|------|------|
| **脑死亡** | `brain ≥ 0.7` | 大脑中枪 ~10 枪（50 伤害步枪）触发；缺氧窒息 ~3 分钟触发 |
| **失血致死** | `blood ≤ 0` | 多伤口未治疗持续出血 → 血量归零。血量 500 以下昏倒，归零死亡 |
| **心脏停跳** | 停跳 **90 秒** | 心脏停跳后 90 秒 → 脑死亡。体温 <28°C 或 >42°C、脉搏 <10 等可致停跳 |
| **肝衰竭** | `liver > 0.5` | 肝功能 > 50% 即开始内出血，完全衰竭（liver=1）约 **5.5 分钟**死亡 |
| **缺氧窒息** | O₂=0 持续 | 肺功能丧失 → 氧气耗尽 → 脑损伤加速 → ~3 分钟脑死亡 |
| **爆头** | 头部累积伤害 > 1000 | 累积大量头部伤害直接爆头击杀 |
| **断颈** | `spine3 ≥ 0.8` + 跌落 | 颈椎严重损伤后跌落直接断颈击杀 |

### 脑损伤来源

| 来源 | 速率/倍率 | 说明 |
|------|----------|------|
| 大脑中枪 | `dmg × 0.006 × 3` | 50 伤害步枪 ≈ 0.09 brain/枪，~10 枪致死 |
| 缺氧（brain<0.3） | `1/350 每秒` | 氧气 < 25% 时开始累积 |
| 缺氧（brain≥0.3） | `1/180 每秒` | 严重缺氧时加速 |
| 颅骨碎裂（被动） | `1/10000 每秒` | 极慢，需配合缺氧 |
| 自然愈合 | `-1/4000 每秒` | brain > 0.1 时缓慢自愈 |
...
O₂=0 → 5 秒后倒地（needfake）
     → O₂<1 时昏倒（needotrub）
     → brain 0→0.1 约 35 秒
     → brain 0.1→0.3 约 77 秒
     → brain 0.3→0.7 约 75 秒
     → 总计 ~3 分钟死亡
```
...
`org.alive = false` → 调用 `owner:Kill()` → 标准 GMod 死亡流程（重生/观战）
...
| `hg_change_posture` | `-1` | 轮切姿态 |
| `hg_change_posture` | `-2` | 重置为自动模式 |
| `hg_unload_ammo` | 无 | 退弹 |
...
手动切换姿势后，系统会记住你的选择，所有武器统一使用该姿势。使用 `hg_change_posture -2` 重置为自动模式。
...
| `zcity_trunk` | 打开后备箱 |
| `hunger_eat` | 进食 |
| `hg_toggle_armor_render` | 切换防弹衣外观显示（默认隐藏） |
...
bind "5" "hg_change_posture -2"   // 重置为自动模式
