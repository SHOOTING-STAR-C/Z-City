...
function MODULE:Init(phys_bone, lpos)
	...
	self.bone_lhand = target:LookupBone("ValveBiped.Bip01_L_Hand")
	self.bone_rhand = target:LookupBone("ValveBiped.Bip01_R_Hand")

	if (self.bone_lhand and self.bone_rhand) then
		self.phys_bone_lhand = target:TranslateBoneToPhysBone(self.bone_lhand)
		self.phys_bone_rhand = target:TranslateBoneToPhysBone(self.bone_rhand)
	end
	...
	for _, bone_name in pairs(allowed_touch) do
		...
		if bone then
			...
			--print("bone",bone_name,bone,phys_bone)
	...
	if phys_bone then
		...
		if (IsValid(phys) and phys_bone != self.phys_bone_lhand and phys_bone != self.phys_bone_rhand) then
			--print(self.phys_bone_lhand,self.phys_bone_rhand)
			--local phys_lhand = target:GetPhysicsObjectNum(14)
			...
			--print("hand",phys_rhand:GetVelocity():Length2D())
			...
			if (IsValid(phys_lhand) and math.random() < grab_chance:GetFloat()) then
				...
				const:SetKeyValue("springaxis", str_axis)
				const:SetKeyValue("constant", 500)
				--const:SetKeyValue("constant", 30000)
				const:SetKeyValue("damping", 0.1)
				const:SetKeyValue("relativedamping", 0.1)
				...
				table.insert(target.Springs, const)
				--print("lhand")
			end
	...
	if(ent.citizen and citizen_hit:GetFloat()>0.5)then
		...
		for k, v in ipairs( ent.hatelist or {} ) do
			if IsValid(v) then
				v:AddEntityRelationship(hole,D_HT,math.floor(math.random()*2)-1)
			end
		end
		hole:Spawn()
		hole:SetHealth(citizen_hit:GetFloat())
		ent.infotarget = hole
		SafeRemoveEntityDelayed(hole, grab_time:GetFloat())
	end
	
	if(ent.player and player_hit:GetFloat()>0.5)then
	...
	if(ent.combine and combine_hit:GetFloat()>0.5)then
		local hole = ents.Create("dummy_info_target_e")
		hole:SetBody(ent)
		--hole:Spawn()
		 for dir,ply in pairs (player:GetAll()) do
			hole:AddEntityRelationship(ply,D_HT,0)
		end
	    ...
	    hole:Spawn()
		hole:SetHealth(combine_hit:GetFloat())
		ent.infotarget = hole
		SafeRemoveEntityDelayed(hole, grab_time:GetFloat())
	end
end
...
local function DoFallingAnim(ent)
	local mod1 = fedhoria.StartModule(ent, "falling_legs", stumble_time:GetFloat())
	local mod2 = fedhoria.StartModule(ent, "falling_torso")
end
function MODULE:Think()
       local ent=self:GetTarget()
	   if IsValid(ent.infotarget) and ent.overkilled then
	   SafeRemoveEntity(ent.infotarget)
	   end
end
...
function MODULE:PhysicsCollide(ent, data)
	...
	if (CurTime() - self.Created < 0.5) then return end
