...
module[2] = function(owner, org, mulTime)
	...
	if org.isPly and not org.otrub and org.blood < 1000 then org.owner:Notify(math.random(2) == 1 and "我感觉不到任何东西了……" or (math.random(2) == 1 and "我觉得我要昏过去了……") or "我感觉不太对劲……",60,"blood2",0) end

	if org.internalBleed < (org.isPly and 5 or 0.5) and org.bleed < (org.isPly and 0.5 or 0.05) and org.pulse > 5 then
	...
	if #org.wounds > 0 then
		...
		for i, wound in pairs(org.wounds) do
			...
			local bleed = rand1 * wound[1] * mulTime * math.max(org.pulse, 20) / 70 * 2.0 * (1 - math.min(adrenaline / 6, 0.5)) * org.bleedingmul * (org.isPly and 0.007 or 0.02)
			local coagulate = 2 * mulTime * rand2 * (adrenaline * 0.1 + 1) * (org.isPly and 0.2 or 0.04)-- / #org.wounds
