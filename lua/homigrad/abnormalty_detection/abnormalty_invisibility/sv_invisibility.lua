	...
	function PLUGIN.Invisibility.UpdateInvisiblity(ply)
		...
		for _, otherPly in player.Iterator() do
			if(IsValid(otherPly) and otherPly ~= ply)then
				local isVisor = ply.Abnormalties_InvisibleVisors[otherPly]
				if(IsValid(ply.FakeRagdoll))then
					recursive_set_prevent_transmit(ply.FakeRagdoll, otherPly, not isVisor)
				end
				recursive_set_prevent_transmit(ply, otherPly, not isVisor)
			end
		end
	end
