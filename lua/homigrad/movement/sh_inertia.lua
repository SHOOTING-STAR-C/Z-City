	...
	hook.Add("SetupMove", "HG(StartCommand)", function(ply, mv, cmd)
		...
		--\\ Friction
			...
			if(CLIENT)then
				...
				ply.MovementInertiaAddView.r = ply.MovementInertiaAddView.r + side_move * delta_time * inertia_len * 0.0003
				ply.MovementInertiaAddView.p = ply.MovementInertiaAddView.p + math.abs(side_move) * delta_time * inertia_len * 0.0001
			end
		--//

		local move = ply:GetRunSpeed() * 1.1
		local noscreenfx = GetConVar("hg_noscreenfx")
		local noNegativeFX = noscreenfx and noscreenfx:GetBool()

		k = 1 * weightmul
		if not noNegativeFX then
		...
		-- Keep positive effects even with noscreenfx
