...
local Compa = CreateConVar("fed_compatibility", "0", FCVAR_ARCHIVE,"")
...
function ENT:Initialize()
	...
	--print("setmdl",self.Module.Model)
	...
	--print("setmdl",target:GetModel())
	--self:SetModel(target:GetModel())
	...
	if bone_rhand then
		...
		self.phys_rhand = target:GetPhysicsObjectNum(phys_bone_rhand)
		if IsValid(self.phys_rhand) then
		local v = self.phys_rhand:GetVelocity()
		local vs = v:Length2D()
		--print("rhand",vs)
		if vs>100 then 
		v:Mul(100/vs) 
		--self.phys_rhand:SetVelocity(v) 
		end
		end
	end
	...
	if bone_lhand then
		...
		self.phys_lhand = target:GetPhysicsObjectNum(phys_bone_lhand)
		if IsValid(self.phys_lhand) then
		local v = self.phys_lhand:GetVelocity()
		local vs = v:Length2D()
		--print("lhand",vs)
		if vs>100 then 
		v:Mul(100/vs) 
		end
		--self.phys_lhand:SetVelocity(v) 
		end
	end	
	--end
	
	local bone_UpperArm = target:LookupBone("ValveBiped.Bip01_L_Forearm")
	if bone_UpperArm then
		local a = target:TranslateBoneToPhysBone(bone_UpperArm)
		self.phys_larm = target:GetPhysicsObjectNum(a)
		--print("self.phys_larm",self.phys_larm:GetID())
	end	
	bone_UpperArm = target:LookupBone("ValveBiped.Bip01_R_Forearm")
	if bone_UpperArm then
		local a = target:TranslateBoneToPhysBone(bone_UpperArm)
		self.phys_rarm = target:GetPhysicsObjectNum(a)
		--print("self.phys_rarm",self.phys_larm:GetID())
	end	
...
function ENT:PhysicsSimulate(phys, dt)
	...
	if target.overkilled2 then
		--if true then return end
	  
	   --factor=factor-0.3
	   factor=0.01
	   if phys:GetID() == self.phys_larm:GetID() then
			--if SERVER then print("sv--",phys:GetID(),factor) end
			return
		end
		if phys:GetID() == self.phys_rarm:GetID() then	
			--if SERVER then print("sv--",phys:GetID(),factor) end		
			return	
		end	
	   
	   if factor<0 then factor=0 end
	   if target.overkilled>5 then
		return end
	elseif target.overkilled then
	   --print(1,factor)
	   factor=1.2
    else
		if self.phys_rhand then
			local v = self.phys_rhand:GetVelocity()
			if v:Length2D()>100 then 
			v:Mul(0.9) 
			self.phys_rhand:SetVelocity(v) 
			end			
		end
		if self.phys_lhand then		
			local v = self.phys_lhand:GetVelocity()
			if v:Length2D()>100 then 
			v:Mul(0.9) 
			self.phys_lhand:SetVelocity(v) 
